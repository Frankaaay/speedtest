...
def pings(target,n,sleep=0.1):
    ...
    run = Popen(["pwsh","-Command",f"1..{n}|Foreach-Object","{(Test-Connection",str(target),"-Count","1",").Latency;", "Start-Sleep",str(sleep),"}",],stdout=PIPE,stderr=PIPE)
    ...
    def run(self):
        ...
        def f():
            ...
            while len(self.res) > self.history:
                self.res.pop(0)
        while True:
            time.sleep(1 - datetime.time().microsecond / 1_000_000)
