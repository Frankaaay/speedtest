...
class SpeedUI:
    ...
    def create_widgets(self):
        ...
        columns = ("地址", "平均", "超时")
        self.statistics = ttk.Treeview(
            self.root, columns=columns, show="headings", height=6
        )
        for col in columns:
    ...
    def update_each_time(self, cnt: int):
        ...
        for address in self.summary.res:
            ...
            self.statistics.item(
                address,
                values=(
                    address,
                    self.summary.res[address].avg,
                    self.summary.res[address].timeout,
                ),
            )
