...
def login_FM_1_3(driver:webdriver.Edge, user:str,password:str):
    ...
    usr = driver.find_element(By.XPATH,'//*[@id="app"]/div/div/div[1]/div/div/div[3]/form/div[1]/div[1]/div/div[1]/div[2]/input')
    usr.clear()
    usr.send_keys(user)
    pwd = driver.find_element(By.XPATH,'//*[@id="app"]/div/div/div[1]/div/div/div[3]/form/div[2]/div[1]/div/div[1]/div[2]/input')
    pwd.clear()
    pwd.send_keys(password)
    driver.find_element(By.XPATH,'//*[@id="app"]/div/div/div[1]/div/div/button').click()
    time.sleep(1)
...
class WebPanel_FM(WebPanel):
    ...
    def update(self):
        ...
        try:
            try:
                res = requests.get(f"http://{self.ip}/xml_action.cgi", params, headers=headers, timeout=self.timeout)
