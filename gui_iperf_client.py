...
class IperfClient:
    ...
    def create_widgets(self):
        ...
        f(tk.Entry(no_name_frame_2, textvariable=self.client_ip, width=13)).pack(
            side=tk.LEFT
        )
        tk.Label(no_name_frame_2, text="端口").pack(side=tk.LEFT)
        self.client_port = tk.StringVar(value="5201")
        f(tk.Entry(no_name_frame_2, textvariable=self.client_port, width=6)).pack(
            side=tk.LEFT
        )
        ...
        f(tk.Entry(no_name_frame_3, textvariable=self.client_duration, width=5)).pack(
            side=tk.LEFT
        )
        ...
        f(tk.Entry(no_name_frame_3, textvariable=self.client_num, width=5)).pack(
            side=tk.LEFT
        )
        ...
        f(tk.Checkbutton(no_name_frame_4, text="UDP", variable=self.use_udp)).pack(
            side=tk.LEFT
        )
        tk.Label(no_name_frame_4, text="带宽").pack(side=tk.LEFT)
        self.client_bandwidth = tk.StringVar(value="")
        f(tk.Entry(no_name_frame_4, textvariable=self.client_bandwidth, width=8)).pack(
            side=tk.LEFT
        )
        ...
        f(tk.Entry(no_name_frame_1, textvariable=self.repeat_count, width=5)).pack(
            side=tk.LEFT
        )
        self.display_only = tk.BooleanVar(value=False)
        f(
            tk.Checkbutton(no_name_frame_1, text="预热", variable=self.display_only)
        ).pack(side=tk.LEFT)
        no_name_frame_1.pack()

        button_frame = tk.Frame(client_frame_common)
        start_button = tk.Button(
            button_frame, text="开始", command=self.start_client_button_clicked
        )
        start_button.pack(side=tk.LEFT)
        stop_button = tk.Button(
            button_frame, text="停止", command=self.stop_client_button_clicked
        )
        stop_button.pack(side=tk.LEFT)
        self.count_label = tk.Label(button_frame, text="第0次")
        self.count_label.pack(side=tk.LEFT)
        copy_button = tk.Button(
            button_frame, text="复制选中", command=self.copy_selected_to_clipboard
        )
    ...
    def start_client_button_clicked(
        self,
    ):
        ...
        self.client_obj = common.Sequence(
            self.client_obj, common.timedelta(seconds=0.5)
        )
    ...
    def stop_client_button_clicked(
        self,
    ):
        if self.client_obj is not None:
